...
class xlsxHelper {
     ...
     */
    var $mysqlServer = DB_HOST;
    var $mysqlUsr = DB_USER;
    var $mysqlPsswrd = DB_PASSWORD;

    function dbConnect($dbase = null) {
    ...
    function set_data($res) {
        while ($row = mysql_fetch_array($res)) {
            ...
            for ($i = 0; $i < $this->num_fields; $i++) {
                ...
                $this->data .= '<ss:Cell><Data ss:Type="' . $p . '">' . utf8_encode(html_entity_decode(mysql_real_escape_string(htmlentities($row[$i])))) . '</Data></ss:Cell>' . chr(10);
            }
            $this->data .= '</ss:Row>' . chr(10);
        }
    ...
    function export() {
        ...
        if ($this->output_file_name == '')
            $this->output_file_name = EXPORT_SYSTEM_DATA . 'data/system/output.xml';
        $this->xml_template = file_get_contents(EXPORT_SYSTEM_DATA . 'template.xml');
